...
export class LoginAttemptsModalComponent extends AppComponentBase {
  ...
  defaultProfilePicture = AppConsts.appBaseUrl + '/assets/common/images/nopicture.png';
  ...
  getLoginAttemptTime(userLoginAttempt: UserLoginAttemptDto): string {
    return (
      moment(userLoginAttempt.creationTime).fromNow() + ' (' + moment(userLoginAttempt.creationTime).format('YYYY-MM-DD hh:mm:ss') + ')'
    );
  }
}
