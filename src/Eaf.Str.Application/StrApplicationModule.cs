...
namespace Eaf.Str
{
    ...
    public class StrApplicationModule : EafModule
    {
        public override void PreInitialize()
        {
            //Adding custom AutoMapper configuration
            Configuration.Modules.EafAutoMapper().Configurators.Add(StrCustomDtoMapper.CreateMappings);
        }

        public override void Initialize()
        {
            IocManager.RegisterAssemblyByConvention(typeof(StrApplicationModule).GetAssembly());
        }

        public override void PostInitialize()
        {
            IocManager.Resolve<EafVirtualFileSystemOptions>().FileSets.AddEmbedded<StrApplicationModule>();
            using (var _connectionStringResolver = IocManager.ResolveAsDisposable<DefaultConnectionStringResolver>())
            {
                ...
                {
                    using (var _migrator = IocManager.ResolveAsDisposable<StrDbMigrator>())
                    {
