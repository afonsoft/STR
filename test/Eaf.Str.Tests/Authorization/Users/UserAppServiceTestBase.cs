    ...
    public abstract class UserAppServiceTestBase : AppTestBase
    {
        ...
        protected User CreateUserEntity(string userName, string name, string surname, string emailAddress)
        {
            var user = new User
            {
                ...
                TenantId = EafSession.TenantId,
                Permissions = new List<UserPermissionSetting>
                {
                    new UserPermissionSetting {Name = "test.permission1", IsGranted = true, TenantId = EafSession.TenantId},
                    new UserPermissionSetting {Name = "test.permission2", IsGranted = true, TenantId = EafSession.TenantId},
                    new UserPermissionSetting {Name = "test.permission3", IsGranted = false, TenantId = EafSession.TenantId},
                    new UserPermissionSetting {Name = "test.permission4", IsGranted = false, TenantId = EafSession.TenantId}
                }
            };
