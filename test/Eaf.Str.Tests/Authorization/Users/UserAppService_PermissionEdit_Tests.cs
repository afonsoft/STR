...
namespace Eaf.Str.Tests.Authorization.Users
{
    ...
    public class UserAppService_PermissionEdit_Tests : UserAppServiceTestBase
    {
        ...
        {
            //Arrange
            var admin = await GetUserByUserNameAsync(EafUserBase.AdminUserName);
        ...
        public async Task Should_Update_User_Permissions()
        {
            ...
            var permissions = Resolve<IPermissionManager>()
                .GetAllPermissions()
                .Where(p => p.MultiTenancySides.HasFlag(EafSession.MultiTenancySide))
                .ToList();
            ...
            UsingDbContext(
                context => context.UserPermissions.Add(
                    new UserPermissionSetting
                    {
                        ...
                        UserId = admin.Id,
                        Name = MiddlewarePermissions.Pages_Administration_Roles,
                        IsGranted = false
                    }));
